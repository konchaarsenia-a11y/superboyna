...
В `gbMe.subscription` дополнительно: `stage` / `stageId` / `stageTitle` / `stageText` / `daysUntil` — клиентские статусы набора (ждём Барни → готовим → в пути → получен).  
...
**Градация жевалок — своя единица учёта у SKU** (`CHEW_STOCK_BY_CUT_ROW_`):

| SKU | 1 шт на складе = | фракции |
|---|---|---|
| Бычий корень | огромный | ОЧ МАЛ 0.0625 · МАЛ 0.125 · СРЕД 0.25 · БОЛ 0.5 · ОГР **1** |
| Трахея | огромный | МАЛ 0.125 · ПЛАСТ 0.25 · СРЕД 0.25 · БОЛ 0.5 · ОГР **1** |
| Становая жила | **большая** | ПАЛК 0.25 · СРЕД 0.5 · БОЛ **1** |
| Ухо Г | **целое обычное** | целое **1** · половинка 0.5 |
| Аорта | **целая** | целая **1** · половинка 0.5 |
| Перепёлки, колени, хрящ, копыто, шеи, губы, носы | 1 шт | без фракции |

**Два вида остатка в миниаппе (`getWarehouse` view / `asOf`):**
- «Начало недели» = F+B (то, от чего `finishFullWeekProduction` спишет всю неделю).
- «На выбранный день» = F+B минус расход дней **строго до** этой даты (утро дня, до нарезки). Галочки нарезки F не списывают.
