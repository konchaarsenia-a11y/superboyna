...
| G…K | **Остаток Пн…Пт** (формулы: `SUM(Прием!C:Q)/(D×1000)`, цепочка от F/B) | лист |
| **L / M** | **Остаток Сб / Остаток Вс** | `setupWarehouseWeekendCols` — **не** copyTo вправо; сдвиг строк C:Q +61/+122 от Пт, prev = K→L |
| K15:K25 | шт-остаток (исторически; не путать с «Остаток Пт» в K1) | → в F при закрытии |
