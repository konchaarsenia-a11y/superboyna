...
6. [VAROK.md](./VAROK.md) — партнёры Варок: заказ пополнения (точки / каталог)
...
Партнёры Варок (заказ пополнения): **`varok/`** — см. `VAROK.md`.  
**Параллельно конвейеру:** агент Варок **никогда** не правит `Code.gs` и `app.html` Бойни (свой Script/книга).
