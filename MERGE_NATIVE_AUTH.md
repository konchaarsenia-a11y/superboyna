...
Канон файла: **[CODE_GS_CANON.md](./CODE_GS_CANON.md)** — один `Code.gs` на `main`; не копировать файл целиком.

**Не копируй весь `Code.gs` с мака на винду** (и не из другого чата/агента).  
Источник правды по бэкенду/вебу — **`Code.gs` на `main`** (актуальный tip веб-агента на Windows).  
