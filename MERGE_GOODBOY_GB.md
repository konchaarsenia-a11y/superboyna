...
| id | Бейдж | Заголовок |
|----|-------|-----------|
| `waiting_stock` | ждём | Ждём, пока Барни сократит запасы лакомств |
...
| `on_the_way` | в пути | Набор уже в пути |
| `delivered` | получен | Набор у вас |
| `trial` / `paused` / `unlinked` | … | пробный / пауза / не привязана |

Логика: дни до следующей даты в `Календарь_Дат` + `calStatus` (read-only). Фронт: `goodboy/js/subscription-status.js`.
