...
function doGet(e) {
  ...
  if (action === "submitGoodboyTry") {
    return handleSubmitGoodboyTry({
      ...
      note: e.parameter.note ? decodeURIComponent(e.parameter.note) : "",
      mode: e.parameter.mode ? decodeURIComponent(e.parameter.mode) : ""
    }, callback, false);
  }
...
function handleSubmitGoodboyTry(json, callback, fromPost) {
  ...
  var mode = String((json && json.mode) || "").trim();
  if (!name || !phone || !pet) {
...
function computeWarehouseWeekPlan_(ss, opts) {
  ...
  for (var i = 0; i < matrix.length; i++) {
    ...
    plan.push(item);
    if (needRaw > 0 && deficit >= (piece ? (gradedChew ? 0.1 : 0.5) : 0.05)) deficits.push(item);
  }
...
function handleTelegramUpdate_(update) {
  try {
    ...
    if (startMatch && !isEdit) {
      ...
      // Goodboy OTP: /start gbotp_<challengeId> — привязка TG и выдача кода (не трогает Доступы)
      if (/^gbotp_/i.test(payload)) {
        try {
          gbBindOtpChallengeFromTelegram_(payload.replace(/^gbotp_/i, ""), from, chat.id, name);
        } catch (eOtp) {
          try { telegramSendText_(chat.id, "Не удалось выдать код входа. Запросите новый на сайте."); } catch (eO2) {}
        }
        return;
      }
...
function partnerFindActiveAccess_(username, tid) {
...
/**
 * V8: @arseniyhotko (Арсений Хотько) — не owner в мини-апп, одна точка Varka · Карского 23.
 * Роль owner в Бойне не трогаем.
 */
function partnerMigrateProdV8_() {
  var props = PropertiesService.getScriptProperties();
  if (props.getProperty("PARTNER_PROD_V8") === "1") return { migrated: false };
  try { partnerMigrateProdV7_(); } catch (e7) {}
  var now = new Date();
  var acSh = getPartnerAccessSheet_();
  var uname = "arseniyhotko";
  var tid = "650923866";
  var pointIds = ["pt_varka_karskogo_23"];
  var rows = readPartnerAccessRows_();
  var hit = null;
  for (var i = 0; i < rows.length; i++) {
  ...
  props.setProperty("PARTNER_PROD_V8", "1");
