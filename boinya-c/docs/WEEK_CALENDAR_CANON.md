...
`upsertMissing` / materialize **не копируют** людей Пн–Вс на «Будущую» / date+7. Клоны 21.09 на 28.09 — `repairFutureWeekDupes` (снять пересечения, затем `alignToGas` под лист «Будущая»). Heal-upsert не чистит extras — overlap-scrub после Future upsert. `forceWeekD1Resync` Future не гонять ради 3 vs 5 (сожмёт слот).  
