...
- **Не клонировать** людей текущей недели на Future/+7 (`upsertMissing` + `findActiveOrderByMatch_` → `continue`, не INSERT). Настоящие брони Future/календаря (GAS или не копия недели) оставлять. Repair: `repairFutureWeekDupes` (только id строки, не `deleteClient`). Бейдж месяца не держит stale `viewDate` выше D1. `deleteClient` с явным `day` не ищет другие слоты через `findActiveOrderByMatch_`.
