...
- **Не клонировать** людей текущей недели на Future/+7 (`upsertMissing` + `findActiveOrderByMatch_` → `continue`, не INSERT). Настоящие брони Future/календаря (GAS или не копия недели) оставлять. Repair: `repairFutureWeekDupes`. Бейдж месяца не держит stale `viewDate` выше D1.
