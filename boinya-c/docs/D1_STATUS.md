...
| Операция | Почему |
|----------|--------|
| **finishFullWeek** / materialize / repair | указатель недели (A1+7), materialize, очистка; Worker **d1-sync** без сдвига `date_iso` (detach + rekey `CAL:mk:date`); после detach — `repairDetachedWeekSlots` + `repairWipedClientFields` |
