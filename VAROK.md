...
1. Mini App читает `@username` → сразу **личный кабинет** (если есть доступ)  
2. В кабинете: **Заказ** · **История** · **Настройки**  
3. Заказ: сначала лакомства/купоны → «Далее» → **только свои точки** → «Отправить»  

Демо без TG: на экране входа выбрать профиль `@varka_two` и т.п.  
Доступы: `ACCESS_BY_USERNAME` во `varka/app.html`.
